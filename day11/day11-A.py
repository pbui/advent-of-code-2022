...
class Monkey:
    def __init__(self, id, items, inspect, check):
        self.id          = id
        self.items       = items
    ...
    def parse_monkey(stream=sys.stdin):
        try:
            ...
            monkey_divisible = int(stream.readline().strip().split()[3])
            monkey_true      = int(stream.readline().strip().split()[5])
            monkey_false     = int(stream.readline().strip().split()[5])
            monkey_check     = lambda lvl: monkey_false if lvl % monkey_divisible else monkey_true
            return Monkey(monkey_id, monkey_items, monkey_inspect, monkey_check)
        except IndexError:
            return None
    ...
    for round in range(1, rounds + 1):
        for monkey in monkeys:
            while monkey.items:
                ...
                new_item   = monkey.inspect(old_item) // 3
...
def main():
    ...
    print(f'Day 11-A: {sussy_monkeys[0].inspections * sussy_monkeys[1].inspections}')
